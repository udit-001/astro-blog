...
Here's the encouraging part: you don't need any planning to start. Just type `opencode` in a terminal and press Enter:
...
That's it — new terminal windows now open in Git Bash. For more on Windows Terminal, see [the official Windows Terminal docs](https://learn.microsoft.com/en-us/windows/terminal/).
...
That's all. uv handles Python for you, no installers or path fiddling. See [docs.astral.sh/uv](https://docs.astral.sh/uv) for more.
...
That's it — typing `python` will now use your real Python instead of opening the Store.
