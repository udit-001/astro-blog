...
A skill is the simplest file you'll ever write: a Markdown file named `SKILL.md`, sitting in a folder, with a little header at the top. It's a text file — no programming involved.
...
That's the whole thing. Two named fields up top (`name` and `description`), then plain-English instructions.
...
Here's a small, satisfying experiment to make it stick — a skill for a monthly ritual many analysts know. The best part: **you don't create any folders or files yourself.** You hand opencode a copy-paste instruction and it does the setup.
...
Skills you install once into your *user* folder (rather than a single project) are available in every project you open from then on. The path is `~/.config/opencode/skills/` — the `~` just means your home folder; on Windows type `%USERPROFILE%` in its place if you ever browse for it.

Here's a genuinely useful one to try — a **grilling** skill that makes opencode interrogate your decisions before you act on them, instead of just agreeing with you. Because some skills reference other files beside the main one, the reliable way to install is to have opencode clone the skill's repo and copy the whole skill folder across:
...
Why it's worth having: agents have a habit of agreeing with whatever you propose. This one pushes back, asks the sharp questions, and stress-tests your thinking before you commit — a genuinely useful check for a data analyst deciding between approaches.

<Callout type="tip">
  That same shape installs almost any shared skill: point the agent at a skill's repo and folder, and it clones, copies, and cleans up for you — no file hunting on your part. Same copy-paste idea you saw in Part 5.
</Callout>

## The big picture

You now control *how* the agent works, not just what it does:

- Parts 1–3: it does tasks.
- **This part:** it learns *your* method for recurring tasks.
- Part 5: it reaches the outside world (web + browser).

That's a genuinely capable assistant — one that does your chores, your way, reliably. You can read more in the [official skills docs](https://opencode.ai/docs/skills/).