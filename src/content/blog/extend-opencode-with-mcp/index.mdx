...
description: "Add MCP servers to opencode — live web search with Exa and browser automation with Playwright"
date: 2026-08-14
tags:
  - opencode
  - mcp
...
This is the single most useful add-on for "answer a current question" work. Without it, opencode answers from memory; with it, it can search the web and read pages in real time — so you can ask *"what are the current revenue numbers for these three companies?"* and get today's answer, not a stale one.
...
That's the whole thing: opencode reads the prompt, adds the config, and tells you it's ready. Prefer to edit by hand? Here's the config it ends up with (mirrors the exact setup this blog uses):
...
- `"type": "remote"` — Exa runs its server online, so opencode just points at it over the internet.
- the `url` — Exa's search server. The `?tools=...` part picks which abilities you want (simple search, page reading, advanced filtered search).
